...
def main():
    ...
    parser.add_argument("--adata", required=True, help="Anndata File for input")
    parser.add_argument("--metadir", required=True, help="Output directory for treatment_well_mapping")
    ...
    data_full.loc[data_full['treatment_id'] > 12, 'treatment_id'] = data_full["treatment_id"] - 1
    ...
    well_mapping = data_full.drop_duplicates(cell_metadata)[['well', 'treatment_id']].sort_values(by='treatment_id').reset_index(drop=True)
    well_mapping.to_csv(meta_outpath)
    
    meta_outpath = metadir / 'treatment_mapping.csv'
    
    treatment_mapping = data_full.drop_duplicates(cell_metadata[['treatment_id', 'treatment']])[['treatment_id', 'treatment']].set_index('treatment_id').sort_index()
